...
3. 首先以 pairing/no-open 模式调用 `launchOrReuseWorkbench`。该 launcher 已按 canonical root 串行化并发启动，因此多个 Codex 任务只能产生一个 owner。
4. 用 `process.ppid` 加父进程启动时间的 hash 形成私有 browser marker。当前 Codex app-server 第一个成功创建 marker 的 bootstrap 再请求一次 ticket bootstrap 并打开浏览器；其余 MCP 进程跳过打开动作。
