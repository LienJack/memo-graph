...
through the ordinary migration/health boundary, verifies every artifact hash,
compares the receipt frontier, closes/checkpoints it, and only then atomically
renames the staging root to the target.
...
- M2+ tombstone, purge, graph/vector projection, release-pointer, and
  cross-version restore frontiers require their later gate evidence.
