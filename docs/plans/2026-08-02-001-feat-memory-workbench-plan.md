...
- **Explicit local trust and launch boundary:** V1 trusts the current OS user and excludes hostile same-UID processes from its threat boundary. Normal launch uses a one-use fragment ticket; browser-open failure or manual no-open recovery reveals a fresh one-use launch URL only on the controlling TTY. Tickets and bearers are never written to logs, files, browser storage, URLs after exchange, argv beyond the one launch handoff, or environment variables.
...
Normal browser launch uses a one-use fragment ticket. If the browser cannot be opened or no-open mode is explicit, a fresh one-use launch URL is revealed only on the controlling TTY; there is no pairing-code form or code-entry API. The current OS user is the accepted local trust boundary for v1, while cross-origin websites, other OS users, stale processes, and reused ports remain adversarial.
...
- For browser-open failure or manual recovery, reveal a fresh one-use fragment launch URL only on the controlling TTY. The base URL never mints authority by itself, and tickets and bearers are not persisted or logged.
...
- Start the host in dependency order, publish non-secret endpoint metadata atomically only after readiness, and open the browser with a shell-free process API. Browser-open failure leaves the host running and reveals one fresh authenticated launch URL only on the controlling TTY.
...
- Error path: launch-ticket replay, expiry, wrong instance, or navigation to the base URL without a valid ticket cannot read memory or create an approval.
