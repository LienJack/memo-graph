  ...
  implements ApprovalRegistry, LearningAuthorityRegistry
{
  ...
  constructor(options: {
    ...
    this.#expectedUid =
      options.expectedUid ?? process.getuid?.();
  }
  ...
  async verify(binding: ApprovalBinding): Promise<VerifiedApproval> {
    ...
    return { grant, registry_hash: snapshot.registry_hash };
  }

  async confirmUnchanged(approval: VerifiedApproval): Promise<void> {
    const snapshot = this.#readSnapshot();
    const current = snapshot.manifest.approvals.find(
      (candidate) => candidate.approval_id === approval.grant.approval_id,
    );
    if (
  ...
  async verifyCanaryAuthorization(
    ...
    const snapshot = this.#readSnapshot();
    const authorization = snapshot.manifest.canary_authorizations?.find(
    ...
    assertCanaryAuthorization(authorization, authorization, verifiedAt);
    return {
      authorization,
  ...
  async confirmCanaryAuthorizationUnchanged(
    ...
    const current = snapshot.manifest.canary_authorizations?.find(
      (candidate) =>
        candidate.authorization_id === verified.authorization.authorization_id,
    );
    if (
      snapshot.registry_hash !== verified.registry_hash ||
      current === undefined ||
      canonicalJson(current) !== canonicalJson(verified.authorization)
    ) {
      throw new ApprovalError("APPROVAL_INVALID");
    }
  }
  ...
  async confirmPostCanaryApprovalUnchanged(
    ...
    const current = snapshot.manifest.post_canary_approvals?.find(
      (candidate) => candidate.approval_id === verified.approval.approval_id,
    );
    if (
      snapshot.registry_hash !== verified.registry_hash ||
      current === undefined ||
