...
/**
 * Reads one private operator-owned file through its already-validated
 * descriptor. The hook is an internal deterministic race-test seam and is
 * never reachable from MCP input.
 */
export function readPrivateOperatorFile(
  ...
  try {
    ...
    if (pathBefore.isSymbolicLink() || !pathBefore.isFile()) {
      throw new Error("trusted recovery key descriptor is invalid");
    }
    descriptor = openSync(
      resolved,
